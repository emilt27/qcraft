...
#[test]
fn create_table_with_identity_full_options() {
    let stmt = SchemaMutationStmt::CreateTable {
        schema: SchemaDef {
            name: "counters".into(),
            namespace: None,
    ...
    assert_eq!(
        render(&stmt),
        r#"CREATE TABLE "counters" ("id" INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY (START WITH 1 INCREMENT BY 2 MINVALUE 1 MAXVALUE 1000 CYCLE CACHE 5))"#
    );
}
...
fn alter_table_add_column() {
    let stmt = SchemaMutationStmt::AddColumn {
        schema_ref: SchemaRef::new("users"),
        column: Box::new(ColumnDef::new("email", FieldType::scalar("TEXT")).not_null()),
        if_not_exists: true,
        position: None,
    };
    assert_eq!(
...
fn create_table_on_commit() {
    use qcraft_core::ast::ddl::OnCommitAction;
