...
impl rquery_core::ast::custom::CustomTableSource for DummyTableSource {
    fn clone_box(&self) -> Box<dyn rquery_core::ast::custom::CustomTableSource> {
        Box::new(self.clone())
    }
...
impl rquery_core::ast::custom::CustomExpr for DummyExpr {
    fn clone_box(&self) -> Box<dyn rquery_core::ast::custom::CustomExpr> {
        Box::new(self.clone())
    }
...
impl rquery_core::ast::custom::CustomCondition for DummyCondition {
    fn clone_box(&self) -> Box<dyn rquery_core::ast::custom::CustomCondition> {
        Box::new(self.clone())
    }
...
impl rquery_core::ast::custom::CustomCompareOp for DummyCompareOp {
    fn clone_box(&self) -> Box<dyn rquery_core::ast::custom::CustomCompareOp> {
        Box::new(self.clone())
    }
    ...
    let stmt = QueryStmt {
        ...
        where_clause: Some(Conditions::and(vec![ConditionNode::Comparison(
            Comparison {
                ...
                right: Expr::Value(rquery_core::ast::value::Value::Int(1)),
                negate: false,
            },
        )])),
        ..simple_query()
